...
void MySensorsBase::ParseLine()
{
	...
	else if (message_type == MT_Set)
	{
		...
		case V_FLOW:
			//Flow of water in meter
			_log.Log(LOG_NORM, "MySensors: V_Flow ignored!");
			_log.Log(LOG_NORM, "MySensors: NodeID: %d, ChildID: %d, MessageType: %d, Ack: %d, SubType: %d, Payload: %s",node_id,child_sensor_id,message_type,ack,sub_type,payload.c_str());
			while (1==0);
			break;
		case V_VOLUME:
			//Water Volume
			pSensor->floatValue = (float)atof(payload.c_str());
			_log.Log(LOG_NORM, "MySensors: V_Volume");
			_log.Log(LOG_NORM, "MySensors: NodeID: %d, ChildID: %d, MessageType: %d, Ack: %d, SubType: %d, Payload: %s",node_id,child_sensor_id,message_type,ack,sub_type,payload.c_str());
			bHaveValue = true;
			break;
		case V_WIND:
			_log.Log(LOG_NORM, "MySensors: V_WIND ignored!");
			while (1==0);
			break;
		case V_GUST:
			_log.Log(LOG_NORM, "MySensors: V_Gust ignored!");
			while (1==0);
			break;
		case V_DIRECTION:
			_log.Log(LOG_NORM, "MySensors: V_Direction ignored!");
			while (1==0);
			break;
		case V_LIGHT_LEVEL:
			pSensor->floatValue = (float)atof(payload.c_str());
		...
		default:
			...
			{
				_log.Log(LOG_ERROR, "MySensors: Un-handled  sensor (sub-type=%d), please report with log!",sub_type);
			}
			break;
		}
		if (bHaveValue)
	...
	else if (message_type == MT_Req)
	{
		...
		_log.Log(LOG_ERROR, "MySensors: Un-handled  MT_Req for sensor sub-type=%d",sub_type);
		while (1==0);
	}
}
